...
public static class PawnShieldGenerator
{
    ...
    private static readonly List<ThingStuffPair> workingShields = [];
    ...
    public static void TryGenerateShieldFor(Pawn pawn, PawnGenerationRequest request)
    {
        ...
        if (workingShields.TryRandomElementByWeight(
                w => w.Commonality * w.Price * getWeaponCommonalityFromIdeo(pawn, w), out var thingStuffPair))
        {
            ...
            var biocodeWeaponChance = request.BiocodeWeaponChance > 0f
    ...
    public static void Reset()
    {
        ...
        {
            return td.equipmentType != EquipmentType.Primary && td.HasComp(typeof(CompShield));
        }
    }
}